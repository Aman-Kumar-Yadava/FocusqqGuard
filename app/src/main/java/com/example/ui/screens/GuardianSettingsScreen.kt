...
fun GuardianSettingsScreen(
    ...
    var allowEarnedTime by remember { mutableStateOf(appSettings.allowEarnedTime) }
            ...
            Column(modifier = Modifier.padding(20.dp)) {
                Row(
                    ...
                    Column(modifier = Modifier.weight(1f)) {
                        Text("Allow Earned Gaming Time", style = MaterialTheme.typography.titleMedium.copy(fontWeight = FontWeight.Bold))
                        Text("Allow study/focus sessions to grant bonus gaming time", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    Switch(
                        checked = allowEarnedTime,
                        onCheckedChange = {
                            allowEarnedTime = it
                            onSaveSettings(appSettings.copy(allowEarnedTime = it))
                        }
                    )
                }
            }
